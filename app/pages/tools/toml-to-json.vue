    ...
    <div class="dualpane no-mid">
      ...
      <div class="pane pane--alt">
        ...
        <div class="pane-body" :class="{ 'pane-body--empty': !output }" :style="output ? 'padding: 0;' : ''" aria-live="polite">
          <template v-if="!output">{{ input.trim() ? 'Fix the error in your input to see JSON output' : 'Paste TOML to see JSON output' }}</template>
          <ClientOnly v-else>
...
useToolSeo(
  ...
  'Convert TOML to JSON instantly. Free online TOML to JSON converter, no data sent to servers.',
)
