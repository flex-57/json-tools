  ...
  <div class="page">
    ...
    <div class="pattern-card" :class="{ 'pattern-card--error': !!regexError }">
      <div class="pattern-row">
        ...
        <div class="pattern-status">
          ...
          <span v-else-if="matches.length > 0 && input" class="match-badge">{{ matches.length }} match{{ matches.length !== 1 ? 'es' : '' }}</span>
    ...
    <div class="editor-card" :class="{ 'drop-target--active': isDragging }" @dragover.prevent="isDragging = true" @dragleave="isDragging = false" @drop.prevent="onDrop">
      ...
      <div class="editor-body">
        <textarea v-model="input" class="pane-textarea" style="padding: 14px 16px;" placeholder="Paste or type your test string here…" spellcheck="false" />
      </div>
    </div>
...
.pattern-slash { font-family: var(--font-mono); font-size: 20px; color: var(--c-t5); line-height: 1; user-select: none; }
.pattern-input { flex: 1; min-width: 160px; font-family: var(--font-mono); font-size: 15px; color: var(--c-t1); background: transparent; border: none; outline: none; }
.pattern-input::placeholder { color: var(--c-t5); }
.flag-group { display: flex; gap: 3px; }
.flag-btn { width: 28px; height: 28px; border-radius: 6px; border: 1px solid var(--c-border); background: var(--c-subtle); font-family: var(--font-mono); font-size: 13px; font-weight: 600; color: var(--c-t3); cursor: pointer; transition: all 0.15s; display: flex; align-items: center; justify-content: center; }
.flag-btn:hover { background: var(--c-faint); color: var(--c-t1); }
.flag-btn--on { background: var(--c-accent); border-color: var(--c-accent); color: var(--c-accent-ink); }
.pattern-status { margin-left: auto; }
.pattern-err-msg { font-family: var(--font-body); font-size: 12px; color: var(--c-error); max-width: 260px; }
...
.highlight-body { padding: 14px 16px; font-family: var(--font-mono); font-size: 12.5px; color: var(--c-t2); line-height: 1.8; white-space: pre-wrap; word-break: break-all; max-height: 300px; overflow: auto; }
:deep(.rx-match) { background: rgb(var(--c-accent-rgb) / 0.25); color: var(--c-accent); border-radius: 3px; padding: 1px 2px; font-weight: 600; }

.matches-body { max-height: 320px; overflow: auto; }
.match-item { padding: 10px 16px; border-bottom: 1px solid var(--c-border-s); }
.match-item:last-child { border-bottom: none; }
.match-item:hover { background: var(--c-faint); }
.match-header { display: flex; align-items: center; gap: 10px; }
.match-index { width: 20px; height: 20px; border-radius: 50%; background: rgb(var(--c-accent-rgb) / 0.1); border: 1px solid rgb(var(--c-accent-rgb) / 0.3); font-family: var(--font-body); font-size: 11px; font-weight: 700; color: var(--c-accent); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.match-value { font-family: var(--font-mono); font-size: 12.5px; background: rgb(var(--c-accent-rgb) / 0.1); color: var(--c-accent); padding: 1px 6px; border-radius: 4px; word-break: break-all; }
.match-pos { font-family: var(--font-mono); font-size: 11px; color: var(--c-t5); margin-left: auto; }
